"""
Created on Mon May 29 10:35:33 2023
...
class User(db.Model):
    ...
    id_organization_area = db.Column(
        db.Integer, db.ForeignKey('IRA_Organization_areas.id_organization_area'),
        nullable=True)
...
questions_vs_networks_modes = db.Table(
    'questions_vs_networks_modes',
    db.Column('id_question', db.Integer, db.ForeignKey('IRA_Questions.id_question')),
    db.Column('id_network_mode', db.Integer,
                db.ForeignKey('IRA_Networks_modes.id_network_mode')))

cycles_vs_networks_modes = db.Table(
    'cycles_vs_networks_modes',
    db.Column('id_cycle', db.Integer, db.ForeignKey('IRA_Cycles.id_cycle')),
    db.Column('id_network_mode', db.Integer,
                db.ForeignKey('IRA_Networks_modes.id_network_mode')))

nodes_vs_networks_modes = db.Table(
    'nodes_vs_networks_modes', db.Column('id_node', db.Integer,
...
class IRA_Adjacency_input_form(db.Model):
    ...
    id_cycle = db.Column(db.Integer, db.ForeignKey('IRA_Cycles.id_cycle'),
                            nullable=False)
    id_network_mode = db.Column(
        db.Integer, db.ForeignKey('IRA_Networks_modes.id_network_mode'),
        nullable=False)
    Is_concluded = db.Column(db.Boolean, nullable=False)
    responses = db.relationship('IRA_Responses',
                                backref=db.backref('adjacency_input_form', lazy=True))
...
class IRA_Cycles(db.Model):
    ...
    networks_modes = db.relationship('IRA_Networks_modes',
                                        ...
                                        backref=db.backref('cycles', lazy='dynamic'))
...
class IRA_Employees_interactions(db.Model):
    ...
    id_cycle = db.Column(db.Integer, db.ForeignKey('IRA_Cycles.id_cycle'),
                            nullable=True)
    id_responding_employee = db.Column(db.Integer, db.ForeignKey('users.id'),
                                        nullable=True)
    id_interacting_employee = db.Column(db.Integer, db.ForeignKey('users.id'),
                                        nullable=True)
...
class IRA_Networks(db.Model):
    ...
    networks_modes = db.relationship('IRA_Networks_modes',
                                        backref=db.backref('network', lazy=False))

    def __repr__(self):
        return f"IRA_Networks('{self.code}', '{self.id}', '{self.name_es}')"

class IRA_Networks_modes(db.Model):
    """sumary_line"""
    __tablename__ = 'IRA_Networks_modes'
    id_network_mode = db.Column(db.Integer, primary_key=True)
    id_network = db.Column(db.Integer, db.ForeignKey('IRA_Networks.id'),
                            nullable=False)
    ...
    #               nullable=True)
    id_node_segment = db.Column(
        db.Integer, db.ForeignKey('IRA_Nodes_segments.id_node_segment'),
        nullable=True)
    id_network_mode_theme = db.Column(
        db.Integer, db.ForeignKey('IRA_Networks_modes_themes.id_network_mode_theme'),
        nullable=True)
    adjacency_forms = db.relationship(
        'IRA_Adjacency_input_form', backref=db.backref('network_mode', lazy=True))
    ...
    def __repr__(self):
        return f"IRA_Networks_modes('{self.id_network_mode}',"\
                ...
                "'{self.id_node_segment}',"\
                "'{self.network.name_es}','{self.id_network}')"
...
class IRA_Networks_modes_themes(db.Model):
    ...
    network_modes = db.relationship(
        'IRA_Networks_modes', backref=db.backref('network_mode_theme', lazy=True))

    def __repr__(self):
        return f"IRA_Networks_modes_themes('{self.id_network_mode_theme}', \
                '{self.code}', '{self.Network_mode_theme_es}', \
                '{self.Network_mode_theme_en}' )"
...
class IRA_Narratives(db.Model):
    ...
    id_employee = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    id_cycle = db.Column(db.Integer, db.ForeignKey('IRA_Cycles.id_cycle'),
                            nullable=False)

    def __repr__(self):
        return f"IRA_Narratives('{self.id}', ' {self.title}','{self.id_employee}',\
                '{self.id_cycle}')"
...
class IRA_Nodes(db.Model):
    ...
    id_node_segment = db.Column(
        ...
        nullable=False)
    id_employee = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=True)
    networks_modes = db.relationship(
        'IRA_Networks_modes', secondary=nodes_vs_networks_modes,
        backref=db.backref('nodes', lazy='dynamic'))

    def __repr__(self):
        return f"IRA_Nodes('{self.id_node}', '{self.Node_es}', \
                '{self.Node_en}', '{self.id_node_segment}', '{self.id_employee}')"
...
class IRA_Nodes_segments(db.Model):
    ...
    id_node_segment_category = db.Column(
        db.Integer, db.ForeignKey('IRA_Nodes_segments_categories.id_node_segment_category'),
        nullable=False)
    nodes = db.relationship('IRA_Nodes', backref=db.backref('node_segment', lazy=True))

    #lo puse aquí
    networks_modes = db.relationship(
        'IRA_Networks_modes', backref=db.backref('node_segment', lazy=True))
...
class IRA_Nodes_segments_categories(db.Model):
    ...
    nodes_segments = db.relationship(
        'IRA_Nodes_segments', backref=db.backref('node_segment_category', lazy=True))
...
class IRA_Organization_areas(db.Model):
    ...
    def __repr__(self):
        return f"IRA_Organization_area('{self.id_organization_area}'," \
                f"'{self.Organization_area_es}', '{self.Organization_area_en}')"
...
class IRA_Questions(db.Model):
    ...
    id_question_possible_answers = db.Column(
        db.Integer, db.ForeignKey('IRA_Questions_possible_answers.id_question_possible_answers'),
        nullable=True)

    networks_modes = db.relationship(
        'IRA_Networks_modes', secondary=questions_vs_networks_modes,
        backref=db.backref('questions', lazy='dynamic'))

    responses = db.relationship(
        'IRA_Responses', backref=db.backref('question', lazy=True))

    def __repr__(self):
        return f"IRA_Questions('{self.id_question}', {self.Question_es}',\
                '{self.Question_en}'," f"'{self.id_question_possible_answers}')"
...
class IRA_Questions_possible_answers(db.Model):
    ...
    questions = db.relationship(
        'IRA_Questions', backref=db.backref('question_possible_answers', lazy=True))

    def __repr__(self):
        return f"IRA_Questions_possible_answers('{self.id_question_possible_answers}'," \
                f"'{self.Question_possible_answers_es}', '{self.multiple}')"
...
class IRA_Responses(db.Model):
    ...
    id_question = db.Column(db.Integer, db.ForeignKey('IRA_Questions.id_question'),
                            nullable=False)
    id_adjacency_input_form = db.Column(
        db.String(15), db.ForeignKey('IRA_Adjacency_input_form.id_adjacency_input_form'),
        nullable=True)
...
class CVF_Culture_input_form(db.Model):
    ...
    id_employee = db.Column(db.Integer, db.ForeignKey('users.id'),
                            nullable=False)
    id_cycle = db.Column(db.Integer, db.ForeignKey('IRA_Cycles.id_cycle'),
                            nullable=False)
    Is_concluded = db.Column(db.Boolean, nullable=False)
    id_culture_mode = db.Column(db.Integer, db.ForeignKey('CVF_Culture_modes.id'),
                                nullable=False)
    db.UniqueConstraint('id_employee', 'id_cycle', 'id_culture_mode',
                        name='uix_1')
    themes_responses = db.relationship(
        'CVF_Themes_responses', backref=db.backref('culture_input_form', lazy=True))

    def __repr__(self):
        return f"CVF_Culture_input_form('{self.id}', '{self.id_employee}', " \
                f"'{self.id_cycle}','{self.id_culture_mode}', '{self.Is_concluded}')"
...
class CVF_Culture_modes(db.Model):
    ...
    culture_modes_themes = db.relationship(
        'CVF_Culture_modes_themes', backref=db.backref('culture_mode', lazy=True))

    # culture_input_forms = db.relationship('CVF_Culture_input_form',
    #                                       backref=db.backref('culture_mode', lazy=True))

    def __repr__(self):
        return f"CVF_Culture_modes('{self.id}', '{self.Culture_mode_es}')"
...
class CVF_Culture_modes_themes(db.Model):
    ...
    id_culture_mode = db.Column(db.Integer, db.ForeignKey('CVF_Culture_modes.id'),
                                nullable=False)
    culture_mode_theme_questions = db.relationship(
        'CVF_Culture_modes_themes_questions', backref=db.backref(
            'culture_mode_theme', lazy=True))
    themes_responses = db.relationship(
        'CVF_Themes_responses', backref=db.backref('culture_mode_theme', lazy=True))
...
class CVF_Culture_modes_themes_questions(db.Model):
    ...
    id_culture_mode_theme = db.Column(
        db.Integer, db.ForeignKey('CVF_Culture_modes_themes.id'), nullable=False)
    id_culture_quadrant = db.Column(
        db.Integer, db.ForeignKey('CVF_Culture_quadrants.id'), nullable=False)
    questions_responses = db.relationship(
        'CVF_Questions_responses', backref=db.backref('modes_themes_question',
                                                        lazy=True))

    def __repr__(self):
        return f"CVF_Culture_modes_themes_questions('" \
                f"'{self.id}'," f"'{self.Culture_mode_theme_question_es}'," \
                f"'{self.id_culture_mode_theme}'," f"'{self.id_culture_quadrant}')"
...
class CVF_Culture_quadrants(db.Model):
    ...
    culture_modes_themes_questions = db.relationship(
        'CVF_Culture_modes_themes_questions', backref=db.backref('culture_quadrant',
                                                                    lazy=True))

    def __repr__(self):
        return f"CVF_Culture_quadrants_es('{self.id}', '{self.Culture_quadrant}')"
...
class CVF_Questions_responses(db.Model):
    ...
    id_theme_responses = db.Column(db.Integer, db.ForeignKey('CVF_Themes_responses.id'),
                                    nullable=False)
    id_culture_mode_theme_question = db.Column(
        db.Integer, db.ForeignKey('CVF_Culture_modes_themes_questions.id'),
        nullable=False)
...
class CVF_Themes_responses(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    id_culture_input_form = db.Column(
        db.Integer, db.ForeignKey('CVF_Culture_input_form.id'), nullable=False)
    id_culture_mode_theme = db.Column(
        db.Integer, db.ForeignKey('CVF_Culture_modes_themes.id'), nullable=False)
    Is_concluded = db.Column(db.Boolean, nullable=False)
    ...
    questions_responses = db.relationship(
        'CVF_Questions_responses', backref=db.backref('theme_responses', lazy=True))

    def __repr__(self):
        return f"CVF_Themes_responses('{self.id}', '{self.id_culture_input_form}'," \
                f"'{self.id_culture_mode_theme}', '{self.Is_concluded}'," \
                f"'{self.Total_actual}','{self.Total_preferred}')"
...
class DW_Roles(db.Model):
    ...
    def __repr__(self):
        return f"DW_Roles('{self.id}'," f"'{self.name_es}','{self.name_en}')"
...
class DW_ServiceUnits(db.Model):
    ...
    def __repr__(self):
        return f"DW_ServiceUnits('{self.id}'," f"'{self.name_es}','{self.name_en}')"
...
class DW_Schools(db.Model):
    ...
    def __repr__(self):
        return f"DW_Schools('{self.id}'," f"'{self.name_es}','{self.name_en}')"
...
class DW_Areas(db.Model):
    ...
    def __repr__(self):
        return f"DW_Areas('{self.id}'," f"'{self.name_es}','{self.name_en}')"
...
class DW_Topics(db.Model):
    ...
    def __repr__(self):
        return f"DW_Topics('{self.id}','{self.name_es}','{self.name_en}')"
...
class DW_Grades(db.Model):
    ...
    def __repr__(self):
        return f"DW_Grades('{self.id}','{self.name_es}','{self.name_en}')"
...
class DW_Sections(db.Model):
    ...
    def __repr__(self):
        return f"DW_Sections('{self.id}','{self.name_es}','{self.name_en}')"
...
class DW_Subjects(db.Model):
    ...
    def __repr__(self):
        return f"DW_Subjects('{self.id}','{self.name_es}','{self.name_en}')"

tools_vs_options = db.Table(
    'DW_tools_vs_options',
...
class DW_Tools(db.Model):
    ...
    options = db.relationship(
        'DW_Options', secondary=tools_vs_options, backref=db.backref(
            'tools', lazy='dynamic'))

    def __repr__(self):
        return f"DW_Tools('{self.id}','{self.name_es}','{self.name_en}')"
...
class DW_Options(db.Model):
    ...
    def __repr__(self):
        return f"DW_Options('{self.id}','{self.name_es}','{self.name_en}')"
...
class DW_GradesSectionsPivot(db.Model):
    ...
    grade_id = db.Column(db.Integer, db.ForeignKey('DW_Grades.id'), primary_key=True)
    section_id = db.Column(db.Integer, db.ForeignKey('DW_Sections.id'),
                            primary_key=True)
    code = db.Column(db.String(20), nullable=True)
...
class DW_GradesSubjectsPivot(db.Model):
    ...
    grade_id = db.Column(db.Integer, db.ForeignKey('DW_Grades.id'), primary_key=True)
    subject_id = db.Column(db.Integer, db.ForeignKey('DW_Subjects.id'),
                            primary_key=True)
